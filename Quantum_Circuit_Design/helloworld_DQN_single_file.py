...
class QuantumEnv(gym.Env):
    ...
    def step(self, action):
        ...
        info_dict = {'gate_count': self.circuit.size(), 'fidelity': reward}  # Track fidelity as info
        return state, reward, done, truncated, info_dict  # False for truncated (not used here)
...
class Config:
    def __init__(self, agent_class=None, env_class=None, env_args=None):
        ...
        self.eval_times = int(8)  # number of times that get episodic cumulative return
...
class Evaluator:
    ...
    def evaluate_and_save(self, actor, horizon_len: int, logging_tuple: tuple):
        ...
        rewards_steps_ary = [get_rewards_and_steps(self.env_eval, actor,if_render = True) for _ in range(self.eval_times)]
...
def get_rewards_and_steps(env, actor, if_render: bool = False) -> (float, int):
    ...
    for episode_steps in range(1234):
...
def train_dqn_for_quantum(gpu_id: int = 0):
    ...
    args.break_step = int(1e5)  # Stop training after 100,000 steps
    args.net_dims = [64, 32]  # Network dimensions for the agent
    args.gamma = 0.95  # Discount factor for rewards

    args.gpu_id = gpu_id  # Set GPU (or CPU if gpu_id is -1)
    train_agent(args)  # Start training the agent

    # Optionally load and render the trained agent
...
if __name__ == '__main__':
    ...
    train_dqn_for_quantum(gpu_id=GPU_ID)
"""
